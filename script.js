"use strict"

function butt(){
	alert('Привет!');
};
	but1.addEventListener("click" ,  butt);

//Задача 2
function butt2(){
	alert('Привет!');
};
	but2.addEventListener("mousemove",butt2 );

//Задача 3
 function butt3(){
	console.log('Привет!');
};
	but3.addEventListener("dblclick",butt3 );
	
//Задача 4
function butt4(){
	alert('Привет!');
}
	but4.addEventListener("mouseout", butt4 );

//Задача 5
function butt5(){
	alert('Ку-ку');		
}
	but5.addEventListener("click",  butt5 );

//Задача 6
function button6click () {
	let but6 = document.getElementById('input6');
	but6.value = 'Поменял текст';
}
   but6.addEventListener('click',button6click);

//Задача 7
function button7click() {
	let but7 = document.getElementById('test7');
	but7.src = 'images/2.jpg' ;		
  }
but7.addEventListener("click",button7click);

//Задача 8
function butt8() {
	let but8 = document.getElementById('but8');
	but8.value = 'Нажми на меня!';
	alert(but8.value);
	}
but8.addEventListener("click", butt8);

//Задача 9
function button9Click() {
	let butt9 = document.getElementById('but9');
	butt9.value = 'Поменяла текст!';
}
but9.addEventListener("click", button9Click);

//Задача 10
function button10Click() {
	let but10 = document.getElementById('but10');
	but10.value = 'Поменяла текст!';
}
but10.addEventListener("click", button10Click);

function mode() {
	let but10 = document.getElementById('but10');
	but10.value = ' Еще раз поменяла текст!';
}
but10.addEventListener("mouseout" , mode);

//Задача 11
function button11Click() {
	let butt11 = document.getElementById('but11');
	butt11.value = 'Поменяла текст!';
}
but11.addEventListener("click", button11Click);

//Задача 12
function button12Click() {
	let inpn = document.getElementById('inp12');
	inpn.disabled = true; 
	inpn.value = 'О, теперь на меня больше не нажать!';
}
inp12.addEventListener("click", button12Click);

//Задача 13
function button13Click(){
	let inpn13 = document.getElementById('img12');
	inpn13.src = 'images/2.jpg';
}
img12.addEventListener("mouseover",button13Click);

function button133Click(){
	let inp13 = document.getElementById('img12');
	inp13.src = 'images/1.jpg';
}
img12.addEventListener("mouseout",button133Click);

//Задача 14
function inp14() {
	let inpp14 = document.getElementById('input14');
	inpp14.value = 'Нажми на меня!';
	inpp14.style.color = 'red';
	inpp14.style.width = '200px';
}
but14.addEventListener("click",  inp14);

//Задача 15
function button15Click() {
	let input15 = document.getElementById('input15');
	input15.style.display = 'none';
}
but15.addEventListener("click",  button15Click);

function button15CClick() {
	let iinput15 = document.getElementById('input15');
	iinput15.style.display = 'inline-block';
}
butt15.addEventListener("click",  button15CClick);

//Задача 16
function button16Click() {
	let input16 = document.getElementById('input16');
	input16.style.color = 'red';
	input16.style.height = '50px';
    input16.style.width = '300px';
    input16.style.borderRadius = '10px';
	input16.value = 'Ой,  я поменял свой текст и css';
	input16.style.display = 'block';
	input16.style.margin = '10px ';
}
but16n.addEventListener("click",  button16Click);

//Задача 17
function button17Click() {
	let inpn17 = document.getElementById('inp17');
	inpn17.disabled = true; 
	inpn17.value = 'О, теперь на меня больше не нажать!';
}
inp17.addEventListener("click", button17Click);

function button17CClick() {
	let inpp17 = document.getElementById('inp17');
	inpp17.style.display = 'none';
}
inp17n.addEventListener("click", button17CClick);

//Задача 18
function but18(elem) {
	elem.value++;	  
}

//Задача 19
// function button19Click() {
//     let inp19 = document.getElementById('but19');
// 	inp19.style.float = 'left';
// 	inp19.style.width = '200px';
// 	let inpp19 = document.getElementById('input19');
// 	inpp19.value= 'Теперь я плавая справа';
// 	inpp19.style.width = '250px';
// }

//Задача 20
function button20Click() {
	let a= 'first';
	let b= 'second';
	let inp20 = document.getElementById('input20');
	inp20.value = 'Мои css классы: ' + `${a } ` + `${b } `;
}
//Задача 21
function buttonClick() {
	let input1 = document.getElementById('input1');
	let input2 = document.getElementById('input2');
	let input1Value = input1.value;
	let input2Value = input2.value;
	input1.value = input2Value;
	input2.value = input1Value;
}
//Задача 22
function button22Click() {
	let iinput22 = document.getElementById('inp22');
	let input22 = document.getElementById('input22');
	let number = Number(inp22.value);
	let square = number*number; 
	input22.value = square;
	input22.disabled = true;
}

//Задача 23
function button23Click() {
	let iinput23 = document.getElementById('inp23');
	let input23 = document.getElementById('input23');
	let number = Number(inp23.value);
	if(typeof input23 == 'number'){
		let square = number*number;
		input23.value = square;
	} else if (typeof input23 != 'number') {
		input23.value = 'Error';
	}
	input23.disabled = true;
}

//Задача 24
function butt24(elem) {
	elem.style.cursor = 'not-allowed';
	elem.disabled = true; 
}
